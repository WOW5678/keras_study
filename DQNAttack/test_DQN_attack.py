# -*- coding: utf-8 -*-
"""
 @Time    : 2019/3/11 0011 下午 9:35
 @Author  : Shanshan Wang
 @Version : Python3.5
 @Function: 测试DQN是是否学习到了愚弄神经网络的策略
"""
import numpy as np
...
if __name__ == '__main__':
    ...
    if dataset == 'MNIST':
        ...
        loaded_Q_model = load_model(dataset + '_Q_adversial_model')
        save_dir = 'data/MNIST_adversarial_examples/'
    ...
    for games in range(5, len(X_test)):
        ...
        for ite in range(max_blocks_attack):
            ...
            attack_region[0, attack_cord[0]:attack_cord[0] + block_size, attack_cord[1]:attack_cord[1] + block_size,
            :] = 1
            ...
            if mod_label != orig_label:
                print('saving adversarial example image')
                ...
                if dataset == 'CIFAR':
                    sample_img_noise.save(save_dir + str(games) + '_actual_' + cifar_class_names[orig_label] + '_mod_' +
                                          cifar_class_names[mod_label] + '.png')

                if dataset == 'MNIST':
                    sample_img_noise.save(
                        save_dir + str(games) + '_actual_' + str(orig_label) + '_mod_' + str(mod_label) + '.png')
